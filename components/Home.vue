        ...
        <div class="div1">
          ...
          <div class="cardCon">
            <div class="tl">
              <div class="tlCon">
                <div class="wrapper">
                  <span data-text="Prisoner's"></span>
                  <span data-text="Dilemma"></span>
                </div>
              </div>
            </div>
    ...
    id="open-modal"
        ...
        <div class="div2 h-full">
          <div class="w-full h-full flex flex-col px-4">
            <!-- Header section -->
            <div class="flex items-center justify-between h-4 pt-4">
              <h2 class="text-sm font-bold text-gray-800">Top players</h2>
...
const { auth, db, dbref, onValue } = useFirebase()
...
// Show info for 7 seconds function
const showInfoTimed = () => {
  showInfo.value = true
  setTimeout(() => {
...
const startGame = () => {
  ...
  const gameConfig = {
    ...
    scenario: scenarios.value.find(s => s.id === selectedScenario.value),
    timestamp: Date.now(),
  }

  // Save game config to Firebase
  const gameRef = dbref(db, `games/${Date.now()}`)
  set(gameRef, {
    ...gameConfig,
  ...
  router.push({
    ...
    query: { config: JSON.stringify(gameConfig) }
  })
...
const handleGameStart = (ev) => {
  if (ev.detail.data) {
    console.log('Game started with config:', ev.detail.data)
  }
}
...
const fetchLeaderboard = () => {
  ...
  onValue(leaderboardRef, (snapshot) => {
    ...
    if (data) {
      ...
      // Sort by score and update topPlayers
      topPlayers.value = players.sort((a, b) => b.score - a.score).slice(0, 5)
    }
  })
}
